<script>
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // Use setDoc for both create and edit
      await setDoc(fileDocRef, {
        ...fileData,
        opvolgdatum: opvolgdatumTimestamp,
        active,
        gekoppelde_facturen: fileData.gekoppelde_facturen.split("\n"),
        ...
        {#each $files as file}
          ...
          >
            ...
            <td class="limit_width">
              {clients.find((client) => client.id === file.client_id)?.label ||
                "Onbekend"}
            </td>
            <td>{file.dossierstatus}</td>
            <td class="hide_mobile">
              {file.opvolgdatum
                ? new Date(file.opvolgdatum.seconds * 1000).toLocaleDateString()
                : "Geen"}
            </td>
